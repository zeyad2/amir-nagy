    ...
    try {
      ...
      switch(type) {
        ...
        case 'tests':
          ...
          break
        case 'files':
          endpoint = 'http://localhost:5000/api/admin/course-files'
          break
      }
              ...
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <span className="text-gray-900">Course Files</span>
                      <p className="text-sm text-gray-600 font-normal">
                  ...
                  <Badge variant="secondary" className="bg-orange-100 text-orange-800">
                    {course.content.files?.length || 0}
                  </Badge>
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                {course.content.files && course.content.files.length > 0 ? (
                  <>
                    <div className="space-y-3 max-h-80 overflow-y-auto">
                      {course.content.files.map((file, index) => (
                        <div
                          key={file.id}
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex items-start space-x-2 flex-1 min-w-0">
                              <FileIcon mimeType={file.mimeType} size={24} className="mt-0.5 flex-shrink-0" />
                              <div className="flex-1 min-w-0">
                                <p className="font-medium text-gray-900 truncate group-hover/item:text-orange-900">
                                  {file.title}
                                </p>
                                <p className="text-xs text-gray-600 truncate">
                                  {file.fileName} • {formatFileSize(parseInt(file.fileSize))}
                                </p>
                              </div>
                            </div>
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => removeContent(file.id, 'file')}
                              className="opacity-0 group-hover/item:opacity-100 text-red-600 hover:text-red-700 hover:bg-red-50 flex-shrink-0 ml-2 transition-all duration-200"
                            >
                              <Minus className="h-4 w-4" />
                            </Button>
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => openContentDialog('files')}
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-gray-600 mb-4">
                      Add files for students to download
                    </p>
                    <Button
                      size="sm"
                      onClick={() => openContentDialog('files')}
